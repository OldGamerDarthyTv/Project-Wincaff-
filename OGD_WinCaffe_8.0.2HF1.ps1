...
Show-OgdStartupIntro -Version '8.0.0'

Clear-Host
Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
Write-Host "  ║      OGD WinCaffè v8.0.0 - DISCLAIMER & CREDITI      ║" -F Cyan
Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Invoke-OgdOfficialRepoUpdateCheck {
    ...
    try{
        Invoke-WebRequest -Uri $latest.DownloadUrl -OutFile $InstalledScript -UseBasicParsing -EA Stop
        Write-Host "  ✓ Script aggiornato a v$($latest.Version) dal repo ufficiale" -F Green
        ...
        try{
            Invoke-WebRequest -Uri $timerRaw -OutFile $timerDst -UseBasicParsing -EA Stop
            Write-Host "  ✓ Timer aggiornato dal repo ufficiale" -F Green
        }catch{}
        Start-Sleep 2
    }catch{
...
Clear-Host
Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
Write-Host "  ║           OGD WinCaffè v8.0.1 - Installazione        ║" -F Cyan
Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Show-Banner {
    ...
    Write-Host "  ║     ▄▄▄▄▄▄▄  ▄▄▄▄▄▄▄  ▄▄▄▄▄▄    ⚡                  ║" -F Yellow
    Write-Host "  ║    █       ██       ██      █                        ║" -F Yellow
Write-Host "  ║    █   ▄   ██   ▄▄▄▄█  ▄    █     WinCaffè v8.0.1    ║" -F Yellow
...
function Show-Banner {
    ...
    Write-Host ""
    Write-Host "  +--------------------------------------------------------------------+" -F Gray
    Write-Host "  | OGD WINCAFFE 8.0.1 // CYBER GAMING WORKBENCH                       |" -F Black -BackgroundColor Gray
    Write-Host "  +--------------------------------------------------------------------+" -F Gray
...
function Show-OgdMainMenu801 {
    ...
    Write-Host "  +------------------------------+  +----------------------------------+" -F Gray
    Write-Host "  | [1] LIGHT                    |  | [6] DNS TNT      [7] EXPLORER   |" -F Green
    ...
    Write-Host "  | [H] HOTFIX 8.0.1                  [J] FIX PRE 8.0.0  [0] ESCI      |" -F Yellow
    Write-Host "  |                                                                    |" -F White
    Write-Host "  | > [2] preset consigliato                                           |" -F DarkGray
    Write-Host "  | > [A] e [C] rami gaming dedicati                                   |" -F DarkGray
    Write-Host "  | > [H] per fix mirati, [J] per bonifica versioni vecchie            |" -F DarkGray
    Write-Host "  +--------------------------------------------------------------------+`n" -F Gray
    Write-Host ""
}
    ...
    switch($Profile){
        'LIGHT' {
            Show-OgdWhatThisDoes 'Questo profilo ha applicato' @(
                'timer, rete, DNS ed Explorer in modalità safe per dare più prontezza senza cambiare troppo Windows',
        ...
        'FIXPRE778' {
Show-OgdWhatThisDoes 'Fix legacy: ramo storico dismesso in 8.0.1HF1' @(
...
function Ensure-OgdFontInterop {
    if(-not ('OGDFontInterop774HF3' -as [type])){
        Add-Type -TypeDefinition @"
using System;
using System.Runtime.InteropServices;
public static class OGDFontInterop774HF3 {
...
function Send-OgdFontChangeBroadcast {
    try{
        ...
        $r = [IntPtr]::Zero
        [OGDFontInterop774HF3]::SendMessageTimeout([OGDFontInterop774HF3]::HWND_BROADCAST,[OGDFontInterop774HF3]::WM_FONTCHANGE,[IntPtr]::Zero,[IntPtr]::Zero,2,1500,[ref]$r) | Out-Null
    }catch{}
}
...
function New-OgdOpenDyslexicRestorePoint {
    $desc = "OGD WinCaffe v7.8.2HF3 OpenDyslexic - $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')"
...
function Repair-OgdOpenDyslexicRegistration {
    ...
    foreach($font in $catalog){
        ...
        if($item.InUser -and (Test-Path $item.UserPath)){
            ...
            try{ [OGDFontInterop774HF3]::AddFontResourceEx($item.UserPath,0,[IntPtr]::Zero) | Out-Null }catch{}
            ...
            try{ [OGDFontInterop774HF3]::AddFontResourceEx($item.SystemPath,0,[IntPtr]::Zero) | Out-Null }catch{}
        }
    }
...
function Install-OgdOpenDyslexic {
    ...
    foreach($font in $catalog){
        ...
        try{
            ...
            try{ [OGDFontInterop774HF3]::AddFontResourceEx($destUser,0,[IntPtr]::Zero) | Out-Null }catch{}
            $installedOne = $true
        }catch{}
        ...
        if(-not $installedOne){
            try{
                ...
                try{ [OGDFontInterop774HF3]::AddFontResourceEx($destSys,0,[IntPtr]::Zero) | Out-Null }catch{}
                $installedOne = $true
            }catch{}
        }
...
function Uninstall-OgdOpenDyslexic {
    ...
    foreach($font in $catalog){
        ...
        foreach($path in @((Join-Path $userFonts $font.File),(Join-Path $sysFonts $font.File))){
            try{ if(Test-Path $path){ [OGDFontInterop774HF3]::RemoveFontResourceEx($path,0,[IntPtr]::Zero) | Out-Null } }catch{}
...
function Invoke-OgdWin24H2Tweaks {
    ...
    if($CreateRestorePoint){
        $desc24 = "OGD WinCaffè v8.0.0 WIN11 24H2+ - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
...
function Invoke-OgdWin10Tweaks {
    ...
    if($CreateRestorePoint){
        $desc10 = "OGD WinCaffè v8.0.0 WIN10 - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
...
function Invoke-OgdSafeNetworkProfile {
    ...
    Write-Success ("Rete safe applicata ({0}): rimossi tweak legacy, NIC ripulite, stack DNS/Winsock riallineato" -f $NetType)
}
...
function Invoke-OgdPre798NetworkDiscordRepair {
    ...
    Show-Banner
    Write-Section "FIX PRE 7.9.8 - RETE / DISCORD"

    if($CreateRestorePoint){
        $desc798 = "OGD WinCaffè v8.0.0 FIX PRE 7.9.8 - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
    ...
    Write-Info "[4] DNS pubblico prudente (Cloudflare / Google)..."
    try{
        Get-DnsClientServerAddress -AddressFamily IPv4 -EA SilentlyContinue |
            Where-Object { $_.InterfaceAlias -and $_.ServerAddresses } |
            ForEach-Object {
                try{ Set-DnsClientServerAddress -InterfaceAlias $_.InterfaceAlias -ServerAddresses @('1.1.1.1','1.0.0.1','8.8.8.8','8.8.4.4') -EA SilentlyContinue }catch{}
            }
    }catch{}
    Write-Success "DNS client riallineato su resolver pubblici stabili"
    ...
    Write-Host ""
    Write-Host "  Questo fix è pensato per chi arriva da tweak rete pre-7.9.8 che hanno lasciato il sistema mezzo offline." -F White
...
function Show-OgdPre798FixMenu {
    while($true){
        Show-Banner
        Write-Section "FIX PRE 7.9.8"
        Write-Host "  [1] Mostra cosa corregge il fix pre 7.9.8" -F White
...
function Show-OgdAddonAppsHint {
    ...
    Write-Host "  +--------------------------------------------------------------------+" -F DarkYellow
    Write-Host "  | Nota: nella 8.0.2 l'installazione resta opzionale e contestuale.   |" -F DarkGray
    Write-Host "  +--------------------------------------------------------------------+" -F DarkYellow
}
...
function Show-OgdHotfixMenu {
    ...
    while($true){
        ...
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
Write-Host "  ║              8.0.0 HOTFIX & ACCESSIBILITÀ             ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Invoke-OgdPre778Repair {
    ...
    if($CreateRestorePoint){
$desc = "OGD WinCaffe v8.0.0 PRE-FIX - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
        New-OgdRestorePoint -Description $desc
    }

    Write-Host ""
Write-Info "Bonifica impostazioni legacy pre-8.0.0..."
    ...
    Write-Host ""
Write-Success "Fix pre-8.0.0 completato"
}
...
function Show-OgdPre778FixMenu {
    while($true){
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
        Write-Host "  ║                FIX PRE 8.0.0 - BONIFICA              ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
'e pensato come ponte pulito verso il ramo 8.0.0 e i profili moderni successivi'
        )
Write-Host '  [1] Mostra cosa corregge il fix pre-8.0.0' -F White
Write-Host '  [2] Applica il fix pre-8.0.0 subito' -F Yellow
...
Write-Host "    • residui legacy poco adatti al ramo 8.0.0" -F DarkGray
                Read-Host '  INVIO per continuare'
            }
            '2' {
...
function New-OgdRestorePoint {
    ...
    try {
        ...
        $rpJob = Start-Job -ScriptBlock {
            param($d)
            Checkpoint-Computer -Description $d -RestorePointType MODIFY_SETTINGS -EA Stop
...
Write-Host "  • Se arrivi da versioni pre-7.9.8 e la rete/Discord non funzionano bene, puoi aprire subito il fix dedicato." -F Yellow
$openPre798Fix = Read-Host "  Aprire ora il menu Fix pre 7.9.8? (S/N)"
...
# Punto ripristino
$desc="OGD WinCaffè v8.0.1 - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
New-OgdRestorePoint -Description $desc
...
Write-Host "  [1] 🟢 LIGHT - Ottimizzazioni base (safe al 100%)" -F White
      Write-Host "      Shell/UI, DNS, rete, Explorer e login più snello senza toccare memoria profonda" -F DarkGray
...
Write-Host "  [H] 🩹 HOTFIX 8.0.2HF1 - DX9 legacy + OpenDyslexic + NPU diag" -F Cyan
Write-Host "  [0] ❌ ESCI - Chiudi script`n" -F Red

$mode=Read-Host "  Scelta (1-9/A/B/F/U/W/N/G/L/P/E/C/M/D/H/J/0)"
...
function Show-OgdBenchmarkMenu {
    while($true){
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
        Write-Host "  ║         BENCHMARK PC & PERSONALIZZAZIONE 8.0.0       ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function Show-OgdUniversalMicroTweaksMenu {
    ...
    while($true){
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
        Write-Host "  ║   MICRO TWEAKS - SVCHOST / THUMBS / CORE 8.0.0      ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
function New-OgdStorageRestorePoint {
    $desc = "OGD WinCaffe v8.0.0 STORAGE - $(Get-Date -Format 'dd/MM/yyyy HH:mm:ss')"
    ...
    if(Get-Command Checkpoint-Computer -EA SilentlyContinue){
        try{ Checkpoint-Computer -Description $desc -RestorePointType 'MODIFY_SETTINGS' -EA SilentlyContinue | Out-Null }catch{}
    }
}
...
function Show-OgdStorageSuperTweaksMenu {
    while($true){
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
        Write-Host "  ║             SSD & NVME SUPER TWEAKS 8.0.0            ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
...
# ═════════════════════════════════════════════════════════════════════════════
#  NETWORK LAB / DNS STORICO / PROFILI ADATTIVI
# ═════════════════════════════════════════════════════════════════════════════
...
function Invoke-OgdNetworkBenchmark {
    ...
    $dnsRanking = foreach($dns in (Get-OgdDnsCatalog)){
        $stats = Test-OgdPingStats -Target $dns.Primary
        [pscustomobject]@{
            Name       = $dns.Name
            Primary    = $dns.Primary
            ...
            Notes      = $dns.Notes
            Score      = [math]::Round(($stats.AvgMs + ($stats.JitterMs * 1.5) + ($stats.PacketLoss * 3)),2)
        }
    }
    $dnsRanking = @($dnsRanking | Sort-Object PacketLoss, Score, AvgMs)
    ...
    $entry = [pscustomobject]@{
        ...
        DnsRanking       = @($dnsRanking)
        SuggestedDns     = if($dnsRanking){ $dnsRanking[0].Name }else{ '' }
    ...
    if($dnsRanking){
        Save-OgdBestNetworkProfile -Profile ([pscustomobject]@{
            ...
            SuggestedDns = $entry.SuggestedDns
...
function Show-OgdNetworkBenchmarkResult {
    ...
    Write-Host ''
    Write-Host '  Ranking DNS suggerito:' -F Cyan
    $i=1
    foreach($d in ($Result.DnsRanking | Select-Object -First 5)){
        Write-Host ("   [{0}] {1} -> avg {2} ms | jitter {3} ms | loss {4}% | {5}" -f $i,$d.Name,$d.AvgMs,$d.JitterMs,$d.PacketLoss,$d.Notes) -F White
        $i++
    }
    Write-Host ''
    Write-Success ("Suggerimento DNS: {0}" -f $Result.SuggestedDns)
...
function Set-OgdDnsForActiveAdapters {
    ...
    foreach($a in $adapters){
        try{
            ...
            Write-Success ("DNS applicato su {0}: {1} / {2}" -f $a.Name,$DnsChoice.Primary,$DnsChoice.Secondary)
        }catch{
            Write-Warning ("DNS non applicato su {0}" -f $a.Name)
        }
    }
}
...
function Reset-OgdNetworkStack {
    Write-Info 'Ripristino stack rete e DNS automatici...'
...
function Show-OgdNetworkHistorySummary {
    ...
    foreach($h in ($history | Select-Object -Last 10)){
        Write-Host ("   {0} -> DNS {1} | Profilo {2}" -f $h.Timestamp,$h.SuggestedDns,$h.SuggestedProfile) -F White
    }
}
...
function Show-OgdNetworkLabMenu {
    while($true){
        Clear-Host
        Write-Host "`n  ╔═══════════════════════════════════════════════════════╗" -F Cyan
        Write-Host "  ║             NETWORK LAB / DNS ADATTIVI 8.0.0         ║" -F Cyan
        Write-Host "  ╚═══════════════════════════════════════════════════════╝`n" -F Cyan
        Write-Host '  [1] Esegui test connessione e ranking DNS' -F Green
        Write-Host '  [2] Scegli e applica DNS in base all ultimo test' -F Yellow
        Write-Host '  [3] Togli DNS personalizzati e torna ad automatico' -F White
        Write-Host '  [4] Applica profilo schede LAN/WiFi consigliato' -F Cyan
        Write-Host '  [5] Ripristina stack rete / reset schede' -F Magenta
        Write-Host '  [6] Ripristina ultimo profilo migliore salvato' -F White
        Write-Host '  [7] Vedi storico test connessione' -F DarkCyan
        Write-Host '  [0] Torna indietro`n' -F DarkGray
        ...
        switch($choice){
            ...
            '2' {
                ...
                Show-OgdNetworkBenchmarkResult -Result $result
                $ranking = @($result.DnsRanking | Select-Object -First 5)
                $idx = 1
                foreach($item in $ranking){
                    Write-Host ("  [{0}] {1} -> {2} / {3}" -f $idx,$item.Name,$item.Primary,$item.Secondary) -F White
                    $idx++
                }
                Write-Host '  [0] Annulla' -F DarkGray
                $dnsPick = Read-Host '  Scegli DNS da applicare'
                if($dnsPick -match '^[1-5]$'){
                    $selected = $ranking[[int]$dnsPick-1]
                    if($selected){
                        Set-OgdDnsForActiveAdapters -DnsChoice $selected
                    }
                }
                Read-Host '  INVIO per continuare'
            }
            ...
            '4' {
                ...
                Save-OgdBestNetworkProfile -Profile ([pscustomobject]@{
                    Timestamp = (Get-Date).ToString('yyyy-MM-dd HH:mm:ss')
                    SuggestedDns = $result.SuggestedDns
            ...
            '6' {
                ...
                if($best){
                    ...
                    if((Read-Host '  Vuoi riapplicare anche il DNS suggerito salvato? (S/N)') -in @('S','s')){
                        $dns = Get-OgdDnsCatalog | Where-Object { $_.Name -eq $best.SuggestedDns } | Select-Object -First 1
                        if($dns){ Set-OgdDnsForActiveAdapters -DnsChoice $dns }
                    }
                } else {
...
if($mode -in @("1","2","3","A","a","4","5","B","b")){
    ...
    Write-Host "  [4] 🧪 NETWORK LAB - Test DNS, storico e reset rete" -F Magenta
    Write-Host "      • Nessun DNS automatico: scegli tu dopo il benchmark" -F DarkGray
    Write-Host "      • Test salvati per confronti futuri e suggerimenti migliori" -F DarkGray
    Write-Host "      • Reset DNS automatici / stack rete / profili adapter`n" -F DarkGray
...
Write-Host "  2. Seleziona: 'OGD WinCaffè v8.0.0'`n" -F White
    Read-Host "  INVIO";continue MenuLoop
}
...
if($mode -in @("F","f")){
    ...
    if($createRestore -in @("S","s")){
        # Punto ripristino
$desc="OGD WinCaffè v8.0.0 FILE I/O - $(Get-Date -Format 'dd/MM/yyyy HH:mm')"
        New-OgdRestorePoint -Description $desc
    }
...
Write-Host "  → Il ramo 8.0.0 evita chiavi NPU vendor-specific non documentate." -F DarkGray
...
Write-Host "  → Il ramo 8.0.0 non forza pagefile o heap tweaks: meglio lasciare gestione automatica salvo casi speciali." -F DarkGray
...
if($mode -in @("1","2","3","A","a","4","5","B","b")){
   ...
   Write-Host "   ✓ Normale Gaming + scheduler avanzato + servizi alleggeriti + storage ottimizzato" -F Magenta
    ...
    if($doBeta){
        ...
        Write-Info "[B3] Rete e cache sicure..."
        ipconfig /flushdns | Out-Null
        netsh winsock reset | Out-Null
        $global:opts++; Write-Success "Rete: cache pulita senza debloat app"
    ...
    if($doLight){
        ...
        if($ult -and $ult.ToString() -match '([a-f0-9-]{36})'){powercfg /setactive $Matches[1] 2>$null}
            ...
            if($ng -match '([a-f0-9-]{36})'){
                powercfg /changename $Matches[1] "Ultimate OGD v8.0.0" 2>$null
                powercfg /setactive $Matches[1] 2>$null
            }
        }
...
'@|Out-File $timerDest2 -Encoding UTF8 -Force
    ...
    Write-Host "`n  ════════════════════════════════════════════════════" -F Cyan
Write-Host "   OGD WinCaffè v8.0.1" -F Yellow
    Write-Host "   #DarkPlayer84Tv Productions" -F Green
    Write-Host "   by OldGamerDarthy Official" -F Green
    Write-Host "  ════════════════════════════════════════════════════`n" -F Cyan
